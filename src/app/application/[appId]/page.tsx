...
import { Copy, Save, Eye, EyeOff, Check, ArrowLeft } from 'lucide-react'
...
export default function AppPage({ params }: { params: Promise<{ appId: string }> }) {
  ...
  const handleScopeChange = (value: string) => {
    setScopeInput(value)
    const scopesArray = value.split(',').map(s => s.trim()).filter(s => s.length > 0)
    setFormData(prev => ({
      ...prev,
      scopes: scopesArray
    }))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
        ...
        <Card className="flex flex-col">
          ...
          <CardContent className="space-y-4 flex-1">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Scopes (virgülle ayırın)</label>
              <Input
                value={scopeInput}
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  size="icon"
                  onClick={() => copyToClipboard(app.client_secret, 'client_secret')}
                >
        ...
        <Card className="flex-1 flex flex-col">
          ...
          <CardContent className="flex items-center gap-4">
            ...
            <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">{app.author.email}</p>
            </div>
          </CardContent>
          <CardFooter>
            <Button variant="outline" className="w-full" onClick={() => router.push(`/profile/${app.author.id}`)}>
                Profili Görüntüle
            </Button>
          </CardFooter>
        </Card>
